...
class TopicsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $categories = Category::where('id', '<', 7)->lists('id')->toArray();

        $faker = app(Faker\Generator::class);

        $topics = factory(Topic::class)->times(rand(10, 20))->make()->each(function ($topic) use ($faker, $users, $categories) {
        ...
        $admin_topics = factory(Topic::class)->times(rand(1, 10))->make()->each(function ($topic) use ($faker, $categories) {
            $topic->user_id     = 1;
            $topic->category_id = $faker->randomElement($categories);
        });
        ...
        $admin_articles = factory(Topic::class)->times(10)->make()->each(function ($topic) use ($faker) {

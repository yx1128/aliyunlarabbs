...
class TopicsController extends Controller implements CreatorListener
{
    ...
    public function create(Request $request)
    {
        ...
        $categories = Category::where(function($query){
                  $query->where('id', '!=', config('phphub.blog_category_id'))
                        ->where('id','!=',  config('phphub.machine_category_id'));
                  })->get();
          ...
          if ($point != 0){
           ...
           if($start != 0 && $end != 0){
            ...
            return view('discussions.show', compact(
                                ...
                                'category', 'banners', 'cover',
                                'votedUsers', 'userTopics', 'revisionHistory','chart','point'));
         }else{
            return view('discussions.show', compact(
                                    'machine', 'user','topic', 'replies', 'categoryTopics',
                                    'category', 'banners', 'cover',
                                    'votedUsers', 'userTopics', 'revisionHistory','point'));
              }
        } else {
    ...
    public function update($id, StoreTopicRequest $request, Mention $mentionParser)
    {
        ...
        if ($topic->isDiscussion() && $request->subject == 'publish' && $topic->is_draft == 'yes') {
            $data['is_draft'] = 'no';
    ...
    public function destroy($id)
    {
        ...
        if ($topic->isArticle() && $topic->is_draft == 'yes') {
            ...
            Auth::user()->decrement('discussion_count', 1);
            $machine->decrement('article_count', 1);
        } else {
